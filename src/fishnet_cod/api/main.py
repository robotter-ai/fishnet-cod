...
@app.get("/views")
async def get_all_views() -> List[View]:
    return await View.fetch_objects().all()


@app.get("/results")
async def get_all_results() -> List[Result]:
...
async def get_user_results(
    ...
    return await Result.where_eq(owner=user_id).page(page=page, page_size=page_size)


@app.get("/userInfo")
async def get_all_userInfo():
