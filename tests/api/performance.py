...
def test_preprocess_ugly_csv(client, account, ugly_big_csv):
    token = login_with_signature(client, account)
    ...
    response = client.post(
        "/timeseries/csv",
        headers={"Authorization": f"Bearer {token}"},
        files={"data_file": ("test.csv", ugly_big_csv, "text/csv")},
    )
    end = time.time()
    print(f"Upload took {end - start} seconds")
    assert end - start < 1
    assert response.status_code == 200
    assert response.num_bytes_downloaded >= ugly_big_csv.__sizeof__()
